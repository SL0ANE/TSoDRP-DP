tag @s add selector
execute store result score @s[type=player] hoverhotbar run data get entity @s SelectedItemSlot

execute as @e[tag=t_mascot] if score @s ownedby = @e[tag=selector,limit=1] playernumber run tag @s add targetmascot
tag @e[tag=picking] remove picking

execute as @e[tag=t_card,scores={handorder=1..}] if score @s ownedby = @e[tag=selector,limit=1] playernumber run tag @s add category1

execute as @e[tag=category1] if score @s handorder = @e[tag=selector,limit=1] hoverhotbar run tag @s add picking
...
execute if score @s hoverhotbar matches 0..7 as @e[tag=picking] at @s unless score system waiting matches 1.. run function lobby:tutorial/float
execute as @e[tag=category1,tag=!picking] at @s unless score system waiting matches 1.. run function lobby:tutorial/hand
