...
item replace entity @e[tag=summon] armor.head from block -182 19 35 container.8
execute as @e[tag=summon] run data modify entity @s CustomName set from block -182 19 35 Items.[{Slot:8b}].tag.display.Name
execute as @e[tag=summon] run data modify entity @s Tags append from block -182 19 35 Items.[{Slot:8b}].tag.Tags.[]
