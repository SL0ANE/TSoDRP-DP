...
item replace entity @e[tag=summon] armor.head from block -182 19 35 container.7
execute as @e[tag=summon] run data modify entity @s CustomName set from block -182 19 35 Items.[{Slot:7b}].tag.display.Name
execute as @e[tag=summon] run data modify entity @s Tags append from block -182 19 35 Items.[{Slot:7b}].tag.Tags.[]
