...
item replace entity @e[tag=summon] armor.head from block -182 18 35 container.6
execute as @e[tag=summon] run data modify entity @s CustomName set from block -182 18 35 Items.[{Slot:6b}].tag.display.Name
execute as @e[tag=summon] run data modify entity @s Tags append from block -182 18 35 Items.[{Slot:6b}].tag.Tags.[]
