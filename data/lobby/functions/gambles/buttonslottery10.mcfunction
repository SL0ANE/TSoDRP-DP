...
execute unless score @a[tag=click,limit=1] money matches 9500.. run tag @s add poor
clear @s white_dye

execute if entity @s[tag=pass,tag=!poor] run scoreboard players remove @a[tag=click] money 9500
execute if entity @s[tag=pass,tag=!poor] run clear @a[tag=click] white_dye

execute if entity @s[tag=pass,tag=!poor] run scoreboard players set lottery system 120
