...
execute as @e[tag=card,scores={cardorder=1..},tag=112,tag=!drawn] if score @s cardorder > @e[tag=drawn,limit=1] cardorder run scoreboard players remove @s cardorder 1
...
playsound minecraft:ui.loom.select_pattern block @a[tag=ingame] ~ ~ ~ 3 1.6 1
