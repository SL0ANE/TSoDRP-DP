...
execute as @e[tag=mascot] unless score @s ownedby = @e[tag=123,limit=1] ownedby run tag @s add targetmascot
execute as @e[tag=unpicked,tag=discovered] run function ingame:insertdeck_top
tag @e remove targetmascot
execute as @e[tag=mascot] if score @s ownedby = @e[tag=123,limit=1] ownedby run tag @s add targetmascot
execute if entity @e[tag=picked] run scoreboard players set @e[tag=targetmascot] step 0
execute if entity @e[tag=picked] run scoreboard players add @e[tag=mascot] drawtime 1
