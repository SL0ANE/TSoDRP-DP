execute unless entity @e[tag=minion,tag=a_precast,tag=!dying,scores={recover=-1}] unless entity @e[tag=minion,tag=a_precast2,tag=!dying,scores={recover=-1}] unless entity @e[tag=a_precast2,tag=normalcaster2] run scoreboard players set @s casting2 19
