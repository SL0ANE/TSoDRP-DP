...
#execute unless entity @e[tag=potentialtrap] unless entity @s[tag=a_preswitch,tag=!dying,scores={recover=-1}] unless entity @e[tag=a_preswitch2,tag=!dying,scores={recover=-1}] unless entity @e[tag=a_preswitch2,tag=normalcaster2] run scoreboard players set @s switching 6
